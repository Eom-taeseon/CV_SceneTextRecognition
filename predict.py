...
from dataset2 import hierarchical_dataset, AlignCollate
...
def benchmark_all_eval(model, criterion, converter, opt):  # , calculate_infer_time=False):
    ...
    for eval_data in eval_data_list:
        ...
        _, accuracy_by_best_model, norm_ED_by_best_model, _, _, _, infer_time, length_of_data, label_ids, pred_results = validation(
            model, criterion, evaluation_loader, converter, opt)

        total_label_ids.extend(label_ids)
        total_preds.extend(pred_results)
    ...
    result_df = pd.DataFrame({'id': total_label_ids, 'predict': total_preds})
...
def validation(model, criterion, evaluation_loader, converter, opt):
    ...
    for i, (image_tensors, labels, label_keys) in enumerate(evaluation_loader):
        ...
        for gt, label_key, pred, pred_max_prob in zip(labels, label_keys, preds_str, preds_max_prob):
            if opt.Transformer:
    ...
    return valid_loss_avg.val(), accuracy, norm_ED, preds_str, confidence_score_list, labels, infer_time, length_of_data, label_ids, pred_results
